...
//升级
    @Update("update bbs_user_table set role='2' where userId=#{id} and role!='3'")
...
//模糊查询
     @Select("<script>select * from bbs_user_table where 1=1 " +"<if test=\"userName !=null \">and " +"userName like '%${userName}%' </if> <if test=\"role !=null \">" + "and role like '%${role}%' </if></script>")
